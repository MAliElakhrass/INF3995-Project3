/*
SupVolumeUpPage:    Libnavajo DynamicPage class that defines the supervisor service 
                    for augmenting volume.

HTTP request : POST /superviseur/<usager>/volume/augmenter/<pc>
*/
...
class SupVolumeUpPage : public DynamicPage
{
public:
    ...
    */
    bool getPage(HttpRequest *request, HttpResponse *response)
    {
        ...
        {
            std::string logErrMsg = "Supervisor " + name + " attempted to turn volume up without being logged in.";
            return logAndRespond(response, logErrMsg, LOGGED_OUT_MSG, UNAUTHENTIFIED_CODE);
        }
        //check request method
        if (request->getRequestType() != POST_METHOD)
        {
            std::string logErrMsg = "Supervisor " + name + " attempted to turn volume up with non POST REST call.";
            return logAndRespond(response, logErrMsg, USE_POST_MSG, WRONG_METHOD_CODE);
        }
        
        SoundControl::getInstance()->volumeUp(pc);

        std::string logMsg = "Supervisor " + name + " has successfully turned volume up.";
